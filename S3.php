...
* S3ForMe PHP class
*
* Based on S3 AWS PHP API 0.2.3
* Modified by Alfredo Ramirez
* 
* @link http://www.ciber-adictos.com/2013/12/subir-archivos-a-s3forme-php-api.html
* @version 0.1.0
*/
class S3 {
	// ACL flags
...
final class S3Request {
	...
	*/
	function __construct($verb, $bucket = '', $uri = '') {
		...
		if ($this->bucket !== '') {
			...
			$this->headers['Host'] = $this->bucket[0].'.rest.s3for.me';
			$this->bucket = implode('/', $this->bucket);
		} else {
			$this->headers['Host'] = 'rest.s3for.me';
	...
	*/
	public function getResponse() {
		...
		// Basic setup
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_USERAGENT, 'S3ForMe-PHP/php');
